...
def process_CLUE():
    ...
    df['canonical_smiles'].dropna().drop_duplicates().to_csv(data_folder+'CLUE/processed_CLUE_smiles.txt',sep='\t',index=0,header=None)
...
def process_DrugComb():
    ...
    df.replace(['0','None'],'').set_index('DrugName').sort_index().to_csv(data_folder+'DrugComb/parsed_DrugComb.tsv',sep='\t')
...
def process_TTD():
    ...
    pd.DataFrame({'TTD_ID':drug_ids}).drop_duplicates().set_index('TTD_ID').sort_index().to_csv(data_folder+'TTD/processed_TTD.txt',sep='\t')

if __name__ == '__main__':
    databases_processors={'BindingDB':process_BindingDB,'BioGRID':process_BioGRID,'Cancerrxgene':process_Cancerrxgene,
                          'CLUE':process_CLUE,'CTDbase':process_CTDbase,'DGIdb':process_DGIdb,'DrugBank':process_DrugBank,
                          'DrugCentral':process_DrugCentral,'DrugComb':process_DrugComb,'DrugCombDB':process_DrugCombDB,
                          'DrugPath':process_DrugPath,'DrugRepurposingHub':process_DrugRepurposingHub,'DsigDB':process_DsigDB,
                          'GtopDB':process_GtopDB,'Hetionet':process_Hetionet,'PDSPKiDB':process_PDSPKIDB,
                          'pharmGKB':process_pharmGKB,'RepoDB':process_RepoDB,'SIDER':process_SIDER,'SMPDB':process_SMPDB,
